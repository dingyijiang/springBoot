...
public class WinUtil {
	...
	public static void KeyDown(int key) throws Exception{ 
		Thread.sleep(20); 
		KBCWait4IBE(); 
		WinIo.Instance.SetPortVal(WinUtil.CONTROL_PORT,0xD2,1); 
		Thread.sleep(10); 
		KBCWait4IBE(); 
		WinIo.Instance.SetPortVal(WinUtil.DATA_PORT,key,1); 
	} 
	public static void KeyUp(int key) throws Exception{ 
		Thread.sleep(10); 
		KBCWait4IBE(); 
		if(WinIo.Instance.SetPortVal(WinUtil.CONTROL_PORT,0xD2,1)) { 
			Thread.sleep(10); 
		} 
		KBCWait4IBE(); 
		WinIo.Instance.SetPortVal(WinUtil.DATA_PORT,(key|0x80),1); 
	}
	
//	public static void KeyDown(int key) throws Exception{
//		KBCWait4IBE();
//		WinIo.Instance.SetPortVal(Selenium.CONTROL_PORT,0xD2,1);
//		KBCWait4IBE();
//		WinIo.Instance.SetPortVal(Selenium.DATA_PORT,key,1);
//	}
//	
//	public static void KeyUp(int key) throws Exception{
//		KBCWait4IBE();
//		WinIo.Instance.SetPortVal(Selenium.CONTROL_PORT,0xD2,1);
//		KBCWait4IBE();
//		WinIo.Instance.SetPortVal(Selenium.DATA_PORT,(key|0x80),1);		
//	}
	...
	public static void clickEvent(String message) throws Exception {
		...
		for (int i = 0; i < message.length(); i++) {
			...
			if(Character.isUpperCase(message.charAt(i))) {
				//大写先转成小写 
				String msg=String.valueOf(message.charAt(i));
				//System.out.println("大写字母="+msg);
				msg=msg.toLowerCase();
				//System.out.println("小写字母="+msg);
				KeyDown(toScanCode("Shift"));
				Thread.sleep(100);
				KeyDown(toScanCode(msg));
				Thread.sleep(300);

				KeyUp(toScanCode("Shift"));
				Thread.sleep(100);
				KeyUp(toScanCode(msg));
				Thread.sleep(300);
			}else {
				KeyDown(toScanCode(""+message.charAt(i)));
				Thread.sleep(100);
				KeyUp(toScanCode(""+message.charAt(i)));
				Thread.sleep(200);
			}
		}
	...
	public static void main(String[] args) throws Exception{  
		System.out.println("winIO64初始化是否成功："+WinIo.Instance.InitializeWinIo());//此处应该有判断，只有初始化成功才可继续往下走，否者直接终止
		Thread.sleep(1000);
		String message="DingyijiangLiJUNFENG123456DINGYIJIANG";
		for (int i = 0; i < message.length(); i++) {
			//大写字母
			if(Character.isUpperCase(message.charAt(i))) {
				//大写先转成小写 
